package ui.common {
	 ...
	 * @author   meps
	 */
	public class CGScroll extends CGProto {
		...
		public function CGScroll(src:* = null, name:String = null) {
			m_enable = true;
			super(src, name);
			...
			m_scrollSlider = new CGResizable(this, SLIDER_ID);
		...
		public function set size(val:Number):void {
			...
			if (val == m_size)
				return;
			m_size = val;
			if (m_position > LIMIT_HIGH - m_size)
		...
		private function redrawScroller():void {
			...
			m_butNext.enable = m_position < LIMIT_HIGH - m_size;
			m_scrollPosition.size = FRAMES_COUNT * m_position;
			m_scrollSlider.size = FRAMES_COUNT * m_size;
		}
		...
		private function onAreaDown(event:MouseEvent):void {
			...
			if (ratio < m_position)
				...
				doPrevStep();
			else if (ratio > m_position + m_size)
		...
		private function onButtonDown(event:CGEvent):void {
			...
			target.stage.addEventListener(MouseEvent.MOUSE_UP, onMouseUp, false, 0, true);
		...
		private var m_position:Number = 0.0;
		private var m_size:Number = 1.0;
		private var m_step:Number = 0.1;
		private var m_area:InteractiveObject; // активная область
		private var m_areaBounds:Rectangle;
		...
		private var m_scrollSlider:CGResizable;
		...
		private static const SLIDER_ID:String = ".position.slider";
		private static const BUTTON_ID:String = ".position.slider.button";
